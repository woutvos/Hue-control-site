...
def get_light(b, light_id):
    ...
    if b == 'example':
        ...
        light = {
            'id': light_id,
            'name': light_raw['name'],
            'status': light_raw['state']['on'],
            'brightness': light_raw['state']['bri'],
            'colormode': light_raw['state']['colormode'],
            'hue': light_raw['state']['hue'],
            'saturation': light_raw['state']['sat'],
            'xy': light_raw['state']['xy'],
            'hex': converter.xy_to_hex(light_raw['state']['xy'][0], light_raw['state']['xy'][1]),
            'colortemp': light_raw['state']['ct'],
            'effect': light_raw['state']['effect'],
            'alert': light_raw['state']['alert'],
            'reachable': light_raw['state']['reachable'],
            'type': light_raw['type']
        }
        ...
        groups_list.append(
            {
                ...
                'lights': {
                    ...
                    'name': [light_names(l, lights) for l in [eval(i) for i in groups_raw[g]['lights']]]
                },
                'type': groups_raw[g]['type'],
                'state': groups_raw[g]['state'],
                'average_brightness': average_brightness(b, groups_raw[g]['lights'])
            },
        )
